...
#### Netty와 이벤트 루프 패턴 
- 대부분의 비동기 웹 서버들이 이벤트 루프를 사용하여 비동기로 동작 (ex: NIO Tomcat, Netty, ...)
- 이벤트 루프: 스레드가 루프를 계속 돌면서 이벤트를 처리하는 방식. 이 루프를 도는 스레드를 이벤트 루프 스레드라고 함
- 이벤트 루프 스레드는 while문(루프)을 계속 돌면서 OS가 쌓아둔 완료된 이벤트가 있는지 확인하고 이벤트가 있으면 이벤트에 붙어 있는 후속 처리 함수를 실행함

![](./images/eventloop.png)

#### Netty와 이벤트 루프 동작 원리
- OS는 이벤트가 완료될 때마다 쌓아둠
- 이벤트 루프 스레드는 1~2를 계속 반복함
  1) 이벤트 루프에서 selector로 OS가 쌓아둔 이벤트가 있는지 조회
  2) 이벤트가 있다면 이벤트에 등록되어 있는 함수를 실행 
...
#### Operator
- Operator란? 연산자
- Operator를 이용해서 Flux, Mono를 시작, 가공, 제어 가능합니다.